...
class SampleTest(unittest.TestCase):
    ...
    def test_crud(self):
        ...
        sample = SampleCriterion.builder().set_col_var('a').set_col_text('b').set_col_tinyint(0).set_col_int(1).set_col_double(2.5) \
            .set_col_datetime(now_str).build()
        entity = sample_dao.insert_sample(sample)
        self.assertIsNotNone(entity.id)

        # read
        criterion_builder = SampleCriterion.builder().col_var_like('a').col_text_in(['b']).col_tinyint_gte(0).col_int_lte(1) \
            .col_double(2.5).col_datetime_start(now_str).col_datetime_end(now_str).page_no(1).page_size(10)
        ...
        criterion = criterion_builder.set_col_var('c').set_col_tinyint(2).set_col_double(3.5).set_col_datetime(one_day_later).build()
        sample_dao.update_sample(criterion)
        criterion = SampleCriterion.builder().col_var_like('c').col_text_in(['b']).col_tinyint_gte(2).col_int_lte(1).col_double(3.5) \
            .col_datetime_start(one_day_later).col_datetime_end(one_day_later).page_no(1).page_size(10).build()
        entities, total = sample_dao.select_sample(criterion)
        self.assertEqual(1, total)
        self.assertEqual(entity.id, entities[0].id)
        ...
        # delete
        criterion_for_delete = SampleCriterion.builder().id(entity.id).build()
        sample_dao.delete_sample(criterion_for_delete)
        entities, total = sample_dao.select_sample(criterion)
    ...
    def test_select(self):
        ...
        criterion = SampleCriterion.builder().col_tinyint_null(reverse=False).build()
        entities, total = sample_dao.select_sample(criterion)
